...
class ControllerPaymentPagseguroBoleto extends Controller {
    ...
    public function test() {
        echo "<pre>";
        var_dump($this->session->data['pagseguro_desconto']);
        
        /* Aplica Desconto */
        if (isset($this->session->data['pagseguro_desconto']))
            var_dump($this->session->data['pagseguro_desconto'] * (-1));
        
        /* Aplica Acréscimo */
        if (isset($this->session->data['pagseguro_acrescimo']))
            var_dump($data['extraAmount'] += $this->session->data['pagseguro_acrescimo']);
    }
}